...
def plot_scene(ds, T_name='T', u_name='mu_u', v_name='mu_v', ax=None, qv_scale=20, qk_size=1.0, qk_x=0.78, qk_y=1.03, cbar=True):
...
def plot_param_series(ds_params, ds_data=None, vel_var='mu_u', vel_idx=(0,0)):
    ...
    if ds_data is not None:
        ...
        ds_data[vel_var].isel(lon=vel_idx[0], lat=vel_idx[1]).plot(ax=ax[0])
        ax[0].set_ylabel(f'{vel_var} [m s$^{{-1}}$]')
        ax[0].set_xlim(ds_data.time[0], ds_data.time[-1])
        ax[0].set_xticklabels([])
...
def plot_prediction_animation(ds, T_name='T', u_name='mu_u', v_name='mu_v', std_u_name='std_u', std_v_name='std_v', qv_scale=20, qk_size=1.0, qk_x=0.78, qk_y=1.03, anim_interval=500):
    """
    Create an animation of the GP regression predictions over time.
    ...
    im0 = ds.isel(time=0)[T_name].plot(ax=ax[0], cmap=cmap, add_colorbar=False)#, vmin=vm, vmax=vx, norm=Normalize(vmin=vm, vmax=vx))
    Q = ax[0].quiver(ds.isel(time=0)['LON'], ds.isel(time=0)['LAT'], 
                    ds.isel(time=0)[u_name], ds.isel(time=0)[v_name],
                    scale=qv_scale)
    im1 = ds.isel(time=0)[std_u_name].plot(ax=ax[1], cmap='Greys', add_colorbar=False, vmin=vm_std, vmax=vx_std)
    im2 = ds.isel(time=0)[std_v_name].plot(ax=ax[2], cmap='Greys', add_colorbar=False, vmin=vm_std, vmax=vx_std)
